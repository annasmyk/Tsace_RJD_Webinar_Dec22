...
\begin{frame}[fragile]{Quick Launch with default specifications}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sa\_13\_v2}\OtherTok{\textless{}{-}}\NormalTok{RJDemetra}\SpecialCharTok{::}\FunctionTok{x13}\NormalTok{(y\_raw, }\AttributeTok{spec =}\StringTok{"RSA5c"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\#v3 \#sa\_x13 \textless- x13(y\_raw, spec= ``RSA5'', context = NULL)
sa\_x13\_v3 \textless- rjd3x13::x13(y\_raw, spec= ``RSA5'')
\end{frame}
...
\begin{frame}[fragile]{Customizing specifications}
...
For the preprocessing step (functions defined in
\texttt{rjd3modelling}):
...
For the decopmostion step (function defined in \texttt{rjd3x13}):
\texttt{set\_x11()}

Adding Benchamarking (like in GUI) (in ?)
\end{frame}

\begin{frame}{Example of customizing specif}
\protect\hypertarget{example-of-customizing-specif}{}
sp = spec\_x13\_default(``rg5c'') y = rjd3toolkit::ABS\$X0.2.09.10.M
fast.x13(y, spec = sp) sp = rjd3modelling::add\_outlier(sp, type =
c(``AO''), c(``2015-01-01'', ``2010-01-01'')) sp =
rjd3modelling::set\_transform( rjd3modelling::set\_tradingdays(
rjd3modelling::set\_easter(sp, enabled = FALSE), option =
``workingdays'' ), fun = ``None'' ) sp = set\_x11(sp, henderson.filter =
13) fast.x13(y, spec = sp)
\end{frame}
